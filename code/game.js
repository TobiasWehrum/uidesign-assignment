            ...
            init: function () {
                ...
                if (Crafty.support.canvas) {
                    ...
                    if (!c) {
                        ...
                        c.style.left = "0px";
                        c.style.top = "0px";
                        c.style.zIndex = '1001000'; //CHANGED
        ...
        Crafty.extend({
            ...
            viewport: {
                ...
                init: function (w, h) {
                    ...
                    Crafty.stage.inner.style.zIndex = "1000000"; //CHANGED
...
//var path = "https://dl.dropboxusercontent.com/u/1977054/projects/games/webfight/";
var path = "http://dragonlab.de/test/";
//var path = "";

function initializeComponents()
{
    Crafty.c("PlatformingPlayer", {
        _speed: null,
        _jumpingVelocity: null,
        _gravityY: null,
        _velocityY: null,
        _airJumpPossibleCount: null,
        _airJumpsStillPossible: null,
        standing: null,
        _animationSpeedFalling: null,
        _animationSpeedJumping: null,
        _animationSpeedWalking: null,
        _animationSpeedIdle: null,
        ...
        _keydown: function (e) {
            ...
            if (e.key == Crafty.keys["UP_ARROW"] || e.key == Crafty.keys["W"]) {
                if (this.standing || (this._airJumpsStillPossible > 0))
                {
                    if (!this.standing) {
                        this._airJumpsStillPossible--;
                    }

                    this.standing = false;
                    this._velocityY = this._jumpingVelocity;
                }
            }
        },
        ...
        _enterframe: function () {
            ...
            var deltaX = this._movement.x;
            var deltaY = this._movement.y + this._velocityY;
            ...
            if (deltaX !== 0) {
                ...
                /*
                var hitData = this.hit("Platform");
                if (hitData) {
                    var maxOverlap = hitData[0].overlap * hitData[0].normal.x;
                    for (var i = 0; i < hitData.length; i++) {
                    */
                        /*
                        var hitObj = hitData[i].obj;
                        if (deltaX > 0) {
                            if (hitObj.x < this.x + this.offsetX + this.collisionWidth) {
                                this.x = hitObj.x - this.offsetX - this.collisionWidth;
                            }
                        } else {
                            if (hitObj.x + hitObj.w  > this.x + this.offsetX) {
                                this.x = hitObj.x + hitObj.w - this.offsetX;
                            }
                        }
                         */
                      /*
                        var maxOverlap = hitData[i].overlap * hitData[i].normal.x;
                        if (Math.abs(overlap) > Math.abs(maxOverlap)) {
                            maxOverlap = overlap;
                        }
                    }
                   */
            ...
            if (deltaY !== 0) {
                ...
                var hitData = this.hit("Platform");
                if (hitData) {
                    var maxOverlap = hitData[0].overlap * hitData[0].normal.y;
                    for (var i = 0; i < hitData.length; i++) {
                    */
                        /*
                        var hitObj = hitData[i].obj;
                        if (deltaY > 0) {
                            if (hitObj.y < this.y + this.offsetY + this.collisionHeight) {
                                this.y = hitObj.y - this.offsetY - this.collisionHeight - 0.1;
                            }
                        } else {
                            if (hitObj.y + hitObj.h  > this.y + this.offsetY) {
                                this.y = hitObj.y + hitObj.h - this.offsetY + 1;
                            }
                        }
                        */
                    /*
                        var overlap = hitData[i].overlap * hitData[i].normal.y;
                        if (Math.abs(overlap) > Math.abs(maxOverlap)) {
                            maxOverlap = overlap;
                        }
                    }
                    ...
                    */
                    ...
                    if (deltaY > 0) {
                        this.standing = true;
                        this._airJumpsStillPossible = this._airJumpPossibleCount;
                    }
                }
            ...
            var actualDeltaX = this.x - oldX;
            var actualDeltaY = this.y - oldY;
            ...
            {
                if (deltaX != 0) {
                    this.animate("PlayerWalk", this._animationSpeedWalking, -1);
                } else {
                    this.animate("PlayerIdle", this._animationSpeedIdle, -1);
                }
            }
            else if (actualDeltaY > 0) {
                this.animate("PlayerFall", this._animationSpeedFalling, -1);
            } else if (actualDeltaY < 0) {
                this.animate("PlayerJump", this._animationSpeedJumping, -1);
            }
        ...
        resolveCollision: function() {
            var hitData = this.hit("Platform");
            if (hitData) {
                var maxOverlapX = hitData[0].overlap * hitData[0].normal.x;
                var maxOverlapY = hitData[0].overlap * hitData[0].normal.y;
                for (var i = 0; i < hitData.length; i++) {
                    var overlapX = hitData[i].overlap * hitData[i].normal.x;
                    if (Math.abs(overlapX) > Math.abs(maxOverlapX)) {
                        maxOverlapX = overlapX;
                    }
                    var overlapY = hitData[i].overlap * hitData[i].normal.y;
                    if (Math.abs(overlapY) > Math.abs(maxOverlapY)) {
                        maxOverlapY = overlapY;
                    }
                }
        ...
        platformingPlayer: function () {
            ...
            this._jumpingVelocity = -6;
            this._gravityY = 0.2;
            this._airJumpPossibleCount = 1;
            this._airJumpsStillPossible = this._airJumpPossibleCount;

            this._velocityY = 0;
            this.standing = false;

            this._animationSpeedFalling = 30;
            this._animationSpeedJumping = 30;
            this._animationSpeedWalking = 15;
            this._animationSpeedIdle = 30;
...
function initializeGame() {
    ...
    //Crafty("cr-stage").style =  Crafty("cr-stage").style + "display: inline;";

    var playerSpeed = 90;

    var centerY = Crafty.DOM.window.width / 2;
    var playerCollisionX = 25;
    var playerCollisionY = 26;
    var playerWidth = 10;
    var playerHeight = 29;
    var platformTop = 50;
    ...
    Crafty.sprite(64, 64, path + 'assets/cat.png', {
        spr_player: [0, 0]
    }, 0, 0);

    Crafty.e("Player, 2D, DOM, PlatformingPlayer, spr_player, SpriteAnimation")
        //.color('rgb(255,0,0)')
        .attr({ x: centerY - playerCollisionX - playerWidth / 2, y: platformTop - playerHeight - playerCollisionY - 10,
            offsetX: playerCollisionX, offsetY: playerCollisionY, collisionWidth: playerWidth, collisionHeight: playerHeight })
        .collision([playerCollisionX, playerCollisionY],
                   [playerCollisionX, playerCollisionY + playerHeight],
                   [playerCollisionX + playerWidth, playerCollisionY + playerHeight],
                   [playerCollisionX + playerWidth, playerCollisionY]
                   )
        .animate("PlayerIdle", 0, 0, 3)
        .animate("PlayerWalk", 0, 1, 7)
        .animate("PlayerJump", 2, 2, 3)
        .animate("PlayerFall", 4, 2, 4)
        .animate("PlayerIdle", 8, -1)
        .platformingPlayer();

    var platform = Crafty.e("Platform, 2D, DOM, Color, Collision")
        .color('rgb(255,0,0)')
        .attr({ x: centerY - platformWidth / 2, y: platformTop, w: platformWidth, h: platformHeight })
        .collision([0, 0],
            [0, platformHeight],
...
function loaded(e)
{
    Crafty.load([path + "assets/cat.png"], function() {
...
function addPlatformsWithTag(tag, predicate) {
    ...
    for (var i=0, max= allElements.length; i < max; i++) {
        ...
        var platform = Crafty.e("Platform, 2D, DOM, Color, Collision")
            .color('rgb(255,0,0)')
            .attr({ x: offset.left, y: offset.top, w: element.offsetWidth, h: element.offsetHeight })
            .collision([0, 0],
...
function createLevel() {
    window.scrollTo(0, 0);
    addPlatformsWithTag("A", null);
    addPlatformsWithTag("IMG", null);
}
